...
### 2. 실시간 데이터 처리
- 실시간 가속도 데이터를 기반으로 차량 움직임을 감지하여 허위 경고를 줄이고 정확성을 높입니다.
...
2. **데이터 처리**
   - Raspberry Pi는 수신된 데이터를 기반으로 가속도 값을 계산하고 이를 통해 차량의 움직임 여부를 분석합니다. 가속도는 다음과 같이 계산됩니다:
...
- **가속도 임계값 (`MOTION_THRESHOLD`)**
    - 기본값: `10g`
    - 사용 환경에 따라 9.9 이상의 값으로 조정합니다.

- **정지 감지 시간 (`STOP_DURATION`)**
    - 기본값: `180`
    - 정차로 간주되기까지의 시간입니다.
